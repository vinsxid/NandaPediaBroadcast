    ...
    if HEROKU_API_KEY is not None:
        ...
        if HEROKU_APP_NAME is None:
            await edit_or_reply(event,
                                ...
                                " untuk dapat deploy perubahan terbaru dari ⚡Pedia Shop⚡.`"
                                )
            repo.__del__()
            return
        ...
        if heroku_app is None:
            await edit_delete(event,
                              f"{txt}\n`Kredensial Heroku tidak valid untuk deploy Pedia Shop dyno.`"
                              )
            return repo.__del__()
        await edit_or_reply(event,
        ...
        if build.status == "failed":
            ...
            await edit_delete(event,
                              "`⚡Pedia Shop⚡ Berhasil DiUpdate✅,Restart Tunggu Sebentar`"
                              )

        if BOTLOG:
            await event.client.send_message(
                BOTLOG_CHATID, "#BOT \n" "`⚡Pedia Shop⚡ Berhasil Di Update`"
            )
...
async def update(event, repo, ups_rem, ac_br):
    ...
    x = await edit_or_reply(event, "**⚡Pedia Shop⚡** `Berhasil Di Update!`")
    await asyncio.sleep(1)
    await x.edit("**⚡Pedia Shop⚡** `Di Restart....`")
    await asyncio.sleep(1)
...
async def upstream(event):
    ...
    if force_update:
        ...
        await xx.edit("` Proses Update ⚡Pedia Shop⚡, Loading....1%`")
        await xx.edit("` Proses Update ⚡Pedia Shop⚡ Loading....20%`")
        await xx.edit("` Proses Update ⚡Pedia Shop⚡, Loading....45%`")
        await xx.edit("` Proses Update ⚡Pedia Shop⚡, Loading....75%`")
        await xx.edit("` Proses Update ⚡Pedia Shop⚡, Updating...90%`")
        await xx.edit(
            "` Proses Update ⚡Pedia Shop⚡, Mohon Tunggu Sebentar....100%`"
        )
...
CMD_HELP.update(
    {
        ...
        \n\n **ᴄᴍᴅ :** `{cmd}update`\
        \n └⋟ Untuk Melihat Pembaruan Terbaru ⚡Pedia Shop⚡.\
        \n\n **ᴄᴍᴅ :** `{cmd}update deploy`\
        \n └⋟ Untuk MengUpdate Fitur Terbaru Dari ⚡Pedia Shop⚡.\
    "
    }
)
